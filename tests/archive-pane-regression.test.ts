...
describe("archived note pane behavior", () => {
  it("keeps the archived note action bar available after reselecting an archived note", () => {
    const source = fs.readFileSync(path.join(process.cwd(), "src/components/use-note-workspace.ts"), "utf8");
    const view = fs.readFileSync(path.join(process.cwd(), "src/components/note-shell-view.tsx"), "utf8");
